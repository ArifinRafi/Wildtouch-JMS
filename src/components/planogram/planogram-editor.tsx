...
export interface CellState { product: string; image: string; qty: number }
export interface RowState { description: string; cells: CellState[] }
export interface SideState { label: string; columns: number; rows: RowState[]; charms: string }

export const emptyCell = (): CellState => ({ product: "", image: "", qty: 0 });

/**
 * The planogram grid editor — shared by "new planogram" (after the setup step)
 * and "edit planogram". Owns the name/sides state; validation + persistence
 * happen through onSave, which returns an error message or null on success.
 */
export function PlanogramEditor({
  ...
  saveLabel: string;
  /** Rendered on the left of the footer (e.g. "Back to setup" or "Cancel"). */
  leftAction?: ReactNode;
  onSave: (name: string, sides: SideState[]) => Promise<string | null>;
}) {
  const [name, setName] = useState(initialName);
  const [sides, setSides] = useState<SideState[]>(initialSides);
  ...
  const save = async () => {
    ...
    const err = await onSave(name.trim(), sides);
  ...
  return (
    <>
      ...
      <div className="space-y-1.5 max-w-md">
        ...
        <Input className="rounded-xl bg-muted/30 border-border/40" value={name} onChange={(e) => setName(e.target.value)} />
      </div>
