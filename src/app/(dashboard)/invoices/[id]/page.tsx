...
.totals .balance{color:#b45309;font-weight:700}
.ptag{display:inline-block;background:#eef2ff;color:#4338ca;border:1px solid #c7d2fe;border-radius:4px;padding:2px 8px;font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;margin:2px 0 8px}
...
<div class="top">
  <div>
    ...
    ${inv.isPartial ? `<div class="ptag">Partial Payment Invoice</div>` : ""}
...
.plano .units{font-size:12px;font-weight:700;color:#3b2f6b}
table.items{width:100%;border-collapse:collapse}
table.items thead th{text-align:left;font-size:12px;font-weight:700;border-bottom:1px solid #cbd5e1;padding:8px 4px}
table.items th.q{text-align:right}
...
.reg{margin-top:26px;font-size:11px;color:#374151;text-align:center;line-height:1.6}
.reg .h{font-weight:700;margin-bottom:3px}
</style></head><body>
<div class="sheet">
<div class="top">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {inv.isPartial && (
              <span className="rounded-lg border border-indigo-500/30 bg-indigo-500/10 px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide text-indigo-600 dark:text-indigo-400">
                Partial · £{(inv.paymentAmount ?? 0).toFixed(2)} of £{(inv.total ?? 0).toFixed(2)}
              </span>
            )}
          </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
        ...
        <motion.aside initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, delay: 0.1 }}
          ...
          <div className="flex items-center justify-between gap-2 mb-1">
            ...
            <button
              onClick={printPlanogram}
              title="Download planogram PDF (no pricing)"
