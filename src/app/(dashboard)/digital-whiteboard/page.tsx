...
export default function DigitalWhiteboardPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const data: WhiteboardOrder[] = await res.json();
        if (on) setOrders(data);
  ...
  const handleSave = useCallback(() => {
    ...
    if (editingId) {
      setOrders((prev) => prev.map((o) => o.id === editingId ? { ...o, ...built } : o)); // optimistic
      fetch(`/api/whiteboard/${editingId}`, {
        method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(built),
      }).catch((e) => console.error(e));
      setDlgOpen(false);
    } else {
      fetch("/api/whiteboard", {
        ...
        .then((created: WhiteboardOrder) => setOrders((prev) => [created, ...prev]))
        .catch((e) => console.error(e));
      setDlgOpen(false);
    }
  }, [form, editingId]);
  ...
  const deleteOrder = (id: string) => {
    ...
    fetch(`/api/whiteboard/${id}`, { method: "DELETE" }).catch((e) => console.error(e));
  };
