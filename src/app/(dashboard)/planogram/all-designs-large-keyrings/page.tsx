...
import { KEYRING_SEGMENTS } from "@/lib/data/large-keyrings";

const SEGS = KEYRING_SEGMENTS;
const SEG_COUNT = SEGS.length; // 7

// ─── 3D cube animation (same as floor stand) ─────────────────────────────────
const cubeVariants = {
...
// ─── State: quantities[segIdx][rowIdx][colIdx] ────────────────────────────────
...
export default function AllDesignsLargeKeyringsPage() {
  ...
  const segTotals = useMemo(
    () =>
      qty.map((seg) =>
        seg.reduce((s, row) => s + row.reduce((a, b) => a + b, 0), 0)
      ),
    [qty]
  );
  ...
  const inc = (ri: number, ci: number) =>
    setVal(activeIdx, ri, ci, segSlots[ri][ci] + 1);
  const dec = (ri: number, ci: number) =>
    setVal(activeIdx, ri, ci, Math.max(0, segSlots[ri][ci] - 1));
  ...
  const handlePrint = useCallback(() => {
    ...
    const colTotals = [0, 1, 2].map((ci) =>
      segSlots.reduce((s, row) => s + row[ci], 0)
    );

    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"/>
<title>Large Keyrings — ${seg.title}</title>
<style>
  @page{size:A4 landscape;margin:15mm}
  *{box-sizing:border-box;margin:0;padding:0}
  body{font-family:"Segoe UI",Arial,sans-serif;font-size:10px;color:#111}
  header{border-bottom:2px solid #7c3aed;padding-bottom:6px;margin-bottom:12px}
  .brand{font-size:16px;font-weight:800;color:#7c3aed}
  .sub{font-size:11px;color:#555;margin-top:2px}
  table{width:100%;border-collapse:collapse}
  th{background:#7c3aed;color:#fff;padding:6px 8px;font-size:9px;text-transform:uppercase;letter-spacing:.05em}
  td{padding:5px 8px;border-bottom:1px solid #e5e7eb;vertical-align:middle}
  .rn{width:30px;text-align:center;font-weight:700;color:#7c3aed}
  .cell{font-weight:500;width:26%}
  .qty{text-align:center;font-weight:700;width:40px;color:#6d28d9}
  .tot{text-align:center;font-weight:700;color:#16a34a;width:50px}
  tfoot td{font-weight:800;border-top:2px solid #7c3aed;background:#f5f3ff}
</style></head><body>
<header>
    ...
    <div className="space-y-6 pb-12">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              7 segments &middot; 14 rows each &middot;{" "}
          ...
          {SEGS.map((s, i) => {
            ...
            return (
              <motion.button
                ...
                onClick={() => goTo(i)}
                className={cn(
                  "relative flex-1 min-w-[90px] rounded-xl px-2 py-2.5 text-xs font-semibold transition-all overflow-hidden whitespace-nowrap",
                  isActive
            ...
            <div className={`bg-gradient-to-r ${seg.color} px-6 py-4 flex items-center justify-between`}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/70">14 rows &middot; 3 product columns</p>
                </div>
              </div>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Grand Total — All Segments</p>
            <p className="text-xs text-muted-foreground/60 mt-0.5">7 segments · 14 rows · 3 columns each</p>
          </div>
        </div>
